...
def _valid_encoding(value: Any) -> Any:
    """Validate encoding."""
    try:
        codec_lookup(value)
    except LookupError:
        raise vol.Invalid("invalid encoding") from None
    return value
...
def _template_url(value: Any) -> Template:
    ...
    try:
        ...
        raise vol.Invalid(str(ex)) from None
    return template
...
OPTIONS_FLOW_SCHEMA = vol.Schema(
    {
        vol.Required(CONF_SCAN_INTERVAL): selector.DurationSelector(
            selector.DurationSelectorConfig(
                enable_day=False, enable_second=True, allow_negative=False
            ),
        )
    }
)
...
SUBENTRY_FLOW_SCHEMA = vol.Schema(
    {
        ...
        vol.Optional(CONF_VALUE_TEMPLATE): vol.All(
            selector.TemplateSelector(), ValueTemplate.from_template
        ),
        vol.Optional(
...
SENSOR_SUBENTRY_FLOW_SCHEMA = SUBENTRY_FLOW_SCHEMA.extend(
    {
        ...
        vol.Optional(CONF_JSON_ATTRS, default=[]): selector.TextSelector(
